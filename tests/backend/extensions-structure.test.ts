...
test("certificate metadata exposes structured extension entries", async () => {
  ...
  const caDerBlock = extractPEMBlock(caPem, "-----BEGIN CERTIFICATE-----", "-----END CERTIFICATE-----");
  ...
  for (const entry of allEntries) {
    ...
    assert.ok(["parsed", "unparsed", "error"].includes(entry.status), "entry status should be normalized");
  ...
  const skiValue = ski?.value && typeof ski.value === "object" && "hex" in ski.value ? (ski.value as { hex: string }).hex : null;
