...
export function buildName(name: pkijs.RelativeDistinguishedNames): Name {
  ...
  for (const tv of name.typesAndValues) {
    ...
    if (blockName.includes("Utf8")) {
      encoding = "utf8String";
    } else if (blockName.includes("Printable")) {
      encoding = "printableString";
    } else if (blockName.includes("IA5")) {
      encoding = "ia5String";
    } else if (blockName.includes("Bmp")) {
      encoding = "bmpString";
    }
  ...
  "1.3.6.1.5.5.7.1.1": (ext): AuthorityInfoAccessValue | undefined => {
    ...
    for (const access of sequence.valueBlock.value) {
      ...
      if (methodOid === "1.3.6.1.5.5.7.48.1") {
        accessMethod = { oid: methodOid, name: "ocsp" };
...
function formatIPv6(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < segments.length; ) {
    ...
    let j = i;
    while (j < segments.length && segments[j] === "0") {
      j++;
    }
    if (j - i > bestLength) {
      bestStart = i;
